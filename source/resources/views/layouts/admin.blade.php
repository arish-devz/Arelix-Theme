    ...
    <head>
        ...
        @php
            ...
            try {
                ...
                $raw = $settingsRepository->get('settings::app:theme:arelix', '{}');
                $decoded = json_decode($raw ?: '{}', true, 512, JSON_THROW_ON_ERROR);
                $themeMeta = $decoded['site']['meta'] ?? null;
                
                // Get admin theme from database
        ...
        @section('scripts')
            ...
            @if($adminTheme === 'arelix')
                {!! Theme::css('css/admin-arelix.css?t={cache-version}') !!}
            @endif
        ...
        @if($themeVars && is_array($themeVars) && count($themeVars) > 0)
            <style id="arelix-theme-vars">
                :root {
                    @foreach($themeVars as $key => $value)
                        @if(Str::startsWith($key, '--arelix-') && !empty($value) && !Str::endsWith($key, '-rgb'))
                            {{ $key }}: {{ $value }}{{ $themeEnforce ? ' !important' : '' }};
                        @endif
                    @endforeach
                    @if(isset($themeVars['--arelix-primary']) && preg_match('/^#([a-f0-9]{6})$/i', $themeVars['--arelix-primary'], $m))
                        --arelix-primary-rgb: {{ hexdec(substr($m[1], 0, 2)) }}, {{ hexdec(substr($m[1], 2, 2)) }}, {{ hexdec(substr($m[1], 4, 2)) }}{{ $themeEnforce ? ' !important' : '' }};
                    @endif
                    @if(isset($themeVars['--arelix-background']) && preg_match('/^#([a-f0-9]{6})$/i', $themeVars['--arelix-background'], $m))
                        --arelix-background-rgb: {{ hexdec(substr($m[1], 0, 2)) }}, {{ hexdec(substr($m[1], 2, 2)) }}, {{ hexdec(substr($m[1], 4, 2)) }}{{ $themeEnforce ? ' !important' : '' }};
                    @endif
                }
            </style>
            @if(isset($themeVars['--arelix-font-url']) && !empty($themeVars['--arelix-font-url']))
                <link rel="stylesheet" href="{{ $themeVars['--arelix-font-url'] }}" media="print" onload="this.media='all'">
                <noscript>
                    <link rel="stylesheet" href="{{ $themeVars['--arelix-font-url'] }}">
                </noscript>
            @endif
        @endif
    </head>
            ...
            <aside class="main-sidebar">
                <section class="sidebar">
                    <ul class="sidebar-menu">
                        ...
                        @endif
                        <li class="header">ARELIX ADDONS</li>
                        <li class="{{ ! starts_with(Route::currentRouteName(), 'admin.extensions.server-splitter') ?: 'active' }}">
                            <a href="{{ route('admin.extensions.server-splitter') }}">
                                <i class="fa fa-columns"></i> <span>Server Splitter</span>
                            </a>
                        </li>
                        <li class="{{ ! starts_with(Route::currentRouteName(), 'admin.extensions.staff-requests') ?: 'active' }}">
                            <a href="{{ route('admin.extensions.staff-requests') }}">
                                <i class="fa fa-ticket"></i> <span>Staff Requests</span>
                            </a>
                        </li>
                        <li class="{{ ! starts_with(Route::currentRouteName(), 'admin.extensions.billing') ?: 'active' }}">
                            <a href="{{ route('admin.extensions.billing') }}">
